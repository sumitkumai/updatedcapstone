...
public class FlightsController {
    ...
    @GetMapping("/search")
    public ResponseEntity<com.edutech.dto.ApiResponse<List<Flights>>> searchFlights(
            @RequestParam String source,
            @RequestParam String destination,
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate date,
            @RequestParam(required = false) Double minPrice,
            @RequestParam(required = false) Double maxPrice,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.TIME) java.time.LocalTime departAfter,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.TIME) java.time.LocalTime departBefore,
            @RequestParam(required = false, defaultValue = "price") String sortBy) {
        return ResponseEntity.ok(com.edutech.dto.ApiResponse.ok(
                flightsService.searchFlightsAdvanced(source, destination, date, minPrice, maxPrice,
                        departAfter, departBefore, sortBy)));
    }
